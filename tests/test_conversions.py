...
async def test_convert_file_pdf_to_docx(
    ...
    files_handler_mock.write.return_value = tmp_path / "a-converted.docx"
    ...
    files_handler_mock.write.assert_called_once_with(
        tmp_path / "a.pdf", b"converted-content", stem_suffix="converted", ext="docx"
    )
...
async def test_convert_file_docx_to_pdf(
    ...
    files_handler_mock.write.assert_called_once_with(
        tmp_path / "doc.docx", b"pdf-output", stem_suffix="converted", ext="pdf"
    )
...
@pytest.mark.anyio
async def test_convert_file_output_written_to_same_directory(
    tool_caller: ToolCaller,
    files_handler_mock: MagicMock,
    platform_handler_mock: MagicMock,
    tmp_path: Path,
...
async def test_convert_file_pdf_to_jpeg_writes_zip(
    ...
    files_handler_mock.write.return_value = tmp_path / "a-converted.zip"
    ...
    files_handler_mock.write.assert_called_once_with(
        tmp_path / "a.pdf", b"zip-with-images", stem_suffix="converted", ext="zip"
    )
...
async def test_convert_file_pdf_to_png_writes_zip(
    ...
    files_handler_mock.read.return_value = b"pdf-content"
    platform_handler_mock.convert_file.return_value = b"zip-with-images"
    files_handler_mock.write.return_value = tmp_path / "doc-converted.zip"
    ...
    files_handler_mock.write.assert_called_once_with(
        tmp_path / "doc.pdf", b"zip-with-images", stem_suffix="converted", ext="zip"
    )
